...
class TorchDataset(Dataset):
    def __init__(self,
    istrain, # if training set 
    fea_dir, # ?
    isaug=False,# set True for training, False for finetuning
    repeat=1 ): # repeat times of dataset
        ...
        self.fea_label_list = self.read_file()  # Which file to read ???   self.istrain
        if self.istrain: # only for training set
    ...
    def __getitem__(self, i):
        index = i % self.len    # the index of video count
