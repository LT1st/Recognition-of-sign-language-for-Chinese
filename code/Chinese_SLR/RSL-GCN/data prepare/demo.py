...
def main():
    with torch.no_grad():
        ...
        for i, path in enumerate(paths):
            print(path)
            paths_ = []
            for r, _, f_ in natsorted(os.walk(path)):
                for fname in natsorted(f_):
                    path1 = os.path.join(r, fname)
                    if 'depth' in fname:
                        continue
                    paths_.append(path1)
            # if i > 1:
            #     break
            output_npy = r'F:\SLRdataset\CSLdataprepare\{}.npy'.format(names[i])
